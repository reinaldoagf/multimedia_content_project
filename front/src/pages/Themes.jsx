...
export default function Themes() {
  ...
  const handleDelete = async () => {
    try {
      // Realizar la solicitud de inicio de sesión utilizando Axios
      const { data } = await _delete(`/themes/${element._id}`);
      getThemes(`/themes`);
      setDefaultValues();
    } catch (error) {
      console.error("Error al enviar la imagen:", error);
    ...
    if (operation && element) {
      switch (operation) {
        case "edit":
          ...
          setCategory(element.category?._id ? element.category._id : "");
  ...
  return (
    <>
      <main className="flex-1 max-h-full p-5 overflow-hidden">
        {/* <!-- Main content header --> */}
        <MainContentHeader title={"Temáticas"} />

        {/* <!-- Table see (https://tailwindui.com/components/application-ui/lists/tables) --> */}
        <h3 className="mt-4 text-xl text-gray-900">Temáticas</h3>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Modal.Body>
              <>
                ...
                <div className="mt-4">
                  <label htmlFor="categories" className="block mb-2 text-sm font-medium">Selecciona una categoría</label>
                  <select
                    id="categories"
                    className="capitalize border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    value={category}
